...
#
# Remove possibly stale configuration file
#
rm -f ~/.config/Nokia/MImEngineWordsInterface.conf
...
case "$TESTCONTEXT" in
	local)
		...
		export LD_LIBRARY_PATH=$TESTBASE/lib:$TESTBASE/../words
		export MYSTUBDRIVER_PATH=$TESTBASE/lib
		OUTSTYLE="plain"
	;;
	*)
